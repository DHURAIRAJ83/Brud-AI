...
def _patch_probe(mgr: RuntimeManager, local_ok: bool, cloud_ok: bool,
    ...
    cloud_models = cloud_models or (["tinyllama", "mistral", "llama3", "qwen2.5:3b"] if cloud_ok else [])
...
async def test_cloud_available():
    ...
    assert "qwen2.5:3b" in mgr._cloud_models
...
async def test_list_models():
    ...
    mgr._cloud_models = ["tinyllama", "mistral", "llama3", "qwen2.5:3b"]
    result = await mgr.list_models()
    assert result["local"] == ["tinyllama", "mistral"]
    assert result["cloud"] == ["tinyllama", "mistral", "llama3", "qwen2.5:3b"]
